...
static void _create_window_emummc()
{
	...
	lv_obj_t *win;
	if (!mbr_ctx.part_idx)
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  파일 기반 에뮤낸드 생성", _action_emummc_window_close);
	else
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  파티션 기반 에뮤낸드 생성", _action_emummc_window_close);
	...
	h_style.body.main_color = LV_COLOR_HEX(0x26333E);
...
static void _create_mbox_emummc_raw()
{
	...
	static const char *mbox_btn_format[] = { "\222계속", "\222취소", "" };
	...
	{
		s_printf(txt_buf,
			"#C7EA46 다음 파티션을 찾았습니다!#\n"
			"#FF8000 계속하려면 파티션을 선택하세요:#\n\n");
	}
	else
		s_printf(txt_buf, "#FFDD00 해당 파티션을 찾지 못했습니다!#\n\n");

	s_printf(txt_buf + strlen(txt_buf),
		"파티션 테이블:\n"
		"#C0C0C0 파티션 0: 유형: %02x, 시작: %08x, 크기: %08x#\n"
		"#%s 파티션 1: 유형: %02x, 시작: %08x, 크기: %08x#\n"
		"#%s 파티션 2: 유형: %02x, 시작: %08x, 크기: %08x#\n"
		"#%s 파티션 3: 유형: %02x, 시작: %08x, 크기: %08x#",
	...
	if (mbr_ctx.resized_cnt[0] || mbr_ctx.resized_cnt[1] || mbr_ctx.resized_cnt[2])
		strcat(txt_buf, "\n\n#FFDD00 안내:# 리사이징한 USER 파티션이 노란색으로 표시.");

	if (!mbr_ctx.available)
		strcat(txt_buf, "\n#FF8000 SD 카드를 분할하시겠습니까?#\n"
						  "#FF8000 (진행 방법 선택 메시지가 표시됩니다)#");
...
static lv_res_t _create_mbox_emummc_create(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\222파일 기반", "\222파티션 기반", "\222취소", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
	...
	lv_mbox_set_text(mbox,
		"#C7EA46 emuMMC# 생성 도구\n\n"
		"생성하려는 에뮤낸드 유형을 선택하세요.\n"
		"#FF8000 파일 기반:# FAT 파티션에 파일로 저장합니다.\n"
		"#FF8000 파티션 기반:# 생성한 파티션에 RAW 이미지로 저장합니다.");
...
static lv_res_t _create_emummc_migrate_action(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map1[] = { "\222파일 기반", "\222파티션 기반", "\222취소", "" };
	...
	if (backup)
	{
		if (!emummc_backup)
			s_printf(txt_buf,
				"#C7EA46 emuMMC 백업을 찾았습니다!#\n\n"
				"#FF8000 마이그레이션 하시겠습니까?#\n");
		else
			s_printf(txt_buf,
				"#C7EA46 emuMMC 백업을 찾았습니다!#\n\n"
				"#FF8000 마이그레이션 하시겠습니까?#\n");
	...
	{
		s_printf(txt_buf,
			"#C7EA46 파티션 기반 emuMMC를 찾았습니다!#\n\n"
			"#FF8000 해당 구성 및 파티션 유형을 복구하시겠습니까?#\n");
		lv_mbox_add_btns(mbox, mbox_btn_map, _create_emummc_mig3_action);
	}
	else if (em_raw && em_file)
	{
		s_printf(txt_buf,
			"#C7EA46 파일 및 파티션 기반 emuMMC를 찾았습니다!#\n\n"
			"#FF8000 마이그레이션할 항목 선택:#\n");
...
static lv_res_t _create_mbox_emummc_migrate(lv_obj_t *btn)
{
	...
	static char *mbox_btn_map[] = { "\262백업", "\262RAW 수정", "\262에뮤낸드", "\222취소", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_SETTINGS"  emuMMC 변경");
	lv_win_add_btn(win, NULL, SYMBOL_POWER"  비활성화", _save_disable_emummc_cfg_action);
...
out0:;
	...
	lv_label_set_static_text(label_txt, "RAW 파티션 기반");
	...
	for (u32 raw_btn_idx = 0; raw_btn_idx < 3; raw_btn_idx++)
	{
		...
		if (!emummc_img->part_sector[raw_btn_idx] || emummc_img->part_type[raw_btn_idx] == 0x83 || !emummc_img->part_path[raw_btn_idx * 128])
		{
			...
			if (emummc_img->part_type[raw_btn_idx] == 0x83)
				lv_label_set_static_text(btn_label, "리눅스");
		}
	...
	lv_label_set_static_text(label_txt3, "파일 기반");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_EDIT"  emuMMC 관리 도구");
	...
	lv_obj_t *label_txt = lv_label_create(h1, NULL);
	lv_label_set_static_text(label_txt, "상태 & 변경");
	lv_obj_set_style(label_txt, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, emu_info.enabled ? "#96FF00 "SYMBOL_OK"  연결 됨!#" : "#FF8000 "SYMBOL_CLOSE"  연결 안 됨!#");
	...
	{
		if (emu_info.sector)
			s_printf(txt_buf, "#00DDFF 유형:# 파티션 기반\n#00DDFF 섹터:# 0x%08X\n#00DDFF Nintendo 폴더:# %s",
				emu_info.sector, emu_info.nintendo_path ? emu_info.nintendo_path : "");
		else
			s_printf(txt_buf, "#00DDFF 유형:# 파일 기반\n#00DDFF 기본 폴더:# %s\n#00DDFF Nintendo 폴더:# %s",
	...
	{
		lv_label_set_static_text(label_txt2, "에뮤낸드가 비활성화 되었으며, eMMC가 부팅에 사용됩니다.\n\n");
	}
	...
	lv_label_set_static_text(label_btn, SYMBOL_SETTINGS"  에뮤낸드 변경");
	...
	lv_label_set_static_text(label_txt2,
		"emuMMC 폴더 또는 SD 카드 파티션에 생성된 이미지 중 선택.\n"
		"최대 3개의 #C7EA46 파티션 기반# 혹은 #C7EA46 파일 기반# 에뮤낸드 중 선택 가능.");
	...
	lv_obj_t *label_txt3 = lv_label_create(h2, NULL);
	lv_label_set_static_text(label_txt3, "도구");
	lv_obj_set_style(label_txt3, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, SYMBOL_DRIVE"  에뮤낸드 생성");
	...
	lv_label_set_static_text(label_txt4,
		"#C7EA46 파일 기반# 혹은 #C7EA46 파티션 기반# 에뮤낸드를 생성합니다.\n"
		"eMMC 또는 백업 낸드에서 생성할 수 있습니다.");
	...
	lv_label_set_static_text(label_btn, SYMBOL_SHUFFLE"  에뮤낸드 이주");
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"#C7EA46 파일 기반#으로 마이그레이션 혹은 #C7EA46 파티션 기반#으로 복구.\n"
		"또는 제 3의 에뮤낸드로 마이그레이션할 수 있습니다.");
	lv_obj_set_style(label_txt4, &hint_small_style);
