...
static int _dump_emummc_file_part(emmc_tool_gui_t *gui, char *sd_path, sdmmc_storage_t *storage, const emmc_part_t *part)
{
	...
	while (totalSectors > 0)
	{
		if (numSplitParts != 0 && bytesWritten >= multipartSplitSize)
		{
			...
			// Replace the previous split file number with the current one.
			const char *label_text = lv_label_get_text(gui->label_info);
			u32 label_length = lv_txt_get_encoded_length(label_text);
			lv_label_cut_text(gui->label_info, label_length - 3, 3);
...
static int _finalize_resized_file_emummc(emmc_tool_gui_t *gui, const char *base_path, u32 resized_count, u32 file_part_sectors)
{
	...
	{
		s_printf(gui->txt_buf, "\n #FFBA00 조 정 된 emuMMC 용 량 이  너 무  작 습 니 다 !#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		return 1;
	}
