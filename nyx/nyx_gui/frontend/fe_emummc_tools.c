...
static int _dump_emummc_file_part(emmc_tool_gui_t *gui, char *sd_path, sdmmc_storage_t *storage, emmc_part_t *part)
{
	...
	{
		s_printf(gui->txt_buf, "\n#FFDD00  사 용  가 능 한  공 간 이  부 족 합 니 다 !#\n");
...
static int _dump_emummc_raw_part(emmc_tool_gui_t *gui, int active_part, int part_idx, u32 sd_part_off, emmc_part_t *part, u32 resized_count)
{
	...
	s_printf(gui->txt_buf, "#96FF00 기본 폴더:#\n%s\n#96FF00 파티션 오프셋:# #FF8000 0x%08X#",
	...
	if (resized_count)
	{
		...
		if (!user_part)
		{
			s_printf(gui->txt_buf, "\n#FFDD00  USER 파 티 션 을  찾 을  수  없 습 니 다 !#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	while (totalSectors > 0)
	{
		...
		{
			s_printf(gui->txt_buf,
				"\n#FFDD00  eMMC 의 %d 개 의  블 록 (@LBA %08X) 을 / 를#\n"
				"#FFDD00  읽 어 오 지  못 했 습 니 다 (%d 개  시 도). #",
				num, lba_curr, ++retryCount);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
		...
		{
			s_printf(gui->txt_buf,
				"\n#FFDD00  SD 카 드 의 %d 블 록 (@LBA %08X) 을 / 를#\n"
				"#FFDD00  작 성 하 지  못 했 습 니 다 (%d 시 도). #",
				num, lba_curr, ++retryCount);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	if (resized_count)
	{
		...
		if (mkfs_error)
		{
			s_printf(gui->txt_buf, "#FF0000  %d 실 패 !#\n 다 시  시 도 하 세 요 ...\n", mkfs_error);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		...
		s_printf(gui->txt_buf, " 새 로 운 GPT 작 성  중 ...");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
		...
		{
			s_printf(gui->txt_buf, "\n#FF0000  USER 파 티 션 을  찾 지  못 했 습 니 다 ...#\n 다 시  시 도 하 세 요 ...\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			free(gpt);
...
static int _emummc_raw_derive_bis_keys(emmc_tool_gui_t *gui, u32 resized_count)
{
	...
	if (memcmp(&cal0->magic, "CAL0", 4))
	{
		...
		strcpy(txt_buf, "#FFDD00 BIS 키 유효성 검사 실패!#\n");
		error = true;
	}
	...
	if (error)
	{
		...
		lv_mbox_set_text(mbox, "#C7EA46 BIS 키 생성#");
...
void dump_emummc_raw(emmc_tool_gui_t *gui, int part_idx, u32 sector_start, u32 resized_count)
{
	...
	{
		s_printf(gui->txt_buf, "#FFDD00  USER 파 티 션 을  포 맷 하 려 면,#\n#FFDD00  BIS 키 가  필 요 합 니 다 !#\n");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		emmc_end();
		goto out;
	}
